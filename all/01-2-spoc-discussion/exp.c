...
trap(int c, int b, int a, int fc, int pc)
{
  ...
  case FIPAGE: printf("FIPAGE [0x%08x]",lvadr()); pc=0; break;
  case FSYS:   printf("FSYS a=0x%x ", a); if(a==0xdead){susp(); fc |= USER;} break;
  case FARITH: printf("FARITH"); break;
  case FMEM:   printf("FMEM [0x%08x]",lvadr()); break;
  case FTIMER: printf("FTIMER"); current = 1; stmr(0); break;
  case FKEYBD: printf("FKEYBD [%c]", in(0)); keypressed = 1; break;
  case FPRIV+USER:  printf("FPRIV"); fc &= ~USER; halt(0); break;
...
//  asm(_dd); // XXX find a better way
  ...
  *(int *)(50<<12) = 5;
  ...
  printf("test instruction page fault...");
  ...
  printf("test privilege exception...");
  asm(LI,0xdead);
  asm(TRAP);
  stmr(0);
  printf("...ok\n");
